    ...
    public class MenuController : Controller
    {
        ...
        public async Task<IActionResult> AddBasket(int id, int menuTableId)
        {
           ...
           CreateBasketDto createBasketDto = new CreateBasketDto
           {
            ...
            var responseMessage = await client.PostAsync("http://localhost:7093/api/Basket", stringContent);

            var client2 = _httpClientFactory.CreateClient();
            //var jsonData2 = JsonConvert.SerializeObject(updateCategoryDto);
            //StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            await client2.GetAsync("http://localhost:7093/api/MenuTables/ChangeMenuTableStatusToTrue?id=" + menuTableId);
